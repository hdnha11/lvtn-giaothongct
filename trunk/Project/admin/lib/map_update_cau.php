...
if (isset($_POST['action'])) {
	...
	if ($_POST['id_duong'] != '') {
		$sqlStr = sprintf("UPDATE %s SET ten='%s', loai='%s', chieu_dai=%u, be_rong=%u, tai_trong=%u, mo_tru='%s',
										 so_nhip=%u, su_dung='%s', su_dung0='%s', id_duong=%u WHERE gid=%u",
		...
		$sqlStr = sprintf("UPDATE %s SET ten='%s', loai='%s', chieu_dai=%u, be_rong=%u, tai_trong=%u, mo_tru='%s',
										 so_nhip=%u, su_dung='%s', su_dung0='%s', id_duong=NULL WHERE gid=%u",
						  $_POST['table'], $_POST['tencau'], $_POST['loai'], $_POST['chieudai'], $_POST['berong'],
						  $_POST['taitrong'], $_POST['motru'], $_POST['sonhip'], $_POST['tinhtrang'], $_POST['tinhchat'],
						  $_POST['id']);
	}
